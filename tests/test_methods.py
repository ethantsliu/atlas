...
class MethodsTests(unittest.TestCase):
    ...
    def test_rejects_tampering(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "curated-family counts"):
            ...
            (output / "candidates.jsonl.gz").write_bytes(
                gzip.compress(payload.encode(), mtime=0)
            )
            changed = copy.deepcopy(index)
            import hashlib

            changed["assets"][0]["sha256"] = hashlib.sha256(
                (output / "candidates.jsonl.gz").read_bytes()
            ).hexdigest()
            (output / "index.json").write_text(json.dumps(changed), encoding="utf-8")
    ...
    def test_schema_workflow(self) -> None:
        ...
        for text in ("contents: write", "git push", "git commit"):
            with self.subTest(text=text):
